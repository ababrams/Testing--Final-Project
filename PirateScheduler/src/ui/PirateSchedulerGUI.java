...
/**
 * GUI for the pirateScheduler project. The GUI displays the course catalog and
 * the student's schedule.
 * 
 * @author Sarah Heckman, Alicia Clery
 */
public class PirateSchedulerGUI extends JFrame implements ActionListener {

	/** ID used for object serialization */
	private static final long serialVersionUID = 1L;
	/** Panel that will contain different views for the application. */
	private JPanel panel;
	/** pirateSchedulerGUI title */
	private static final String APP_TITLE = "PirateScheduler";
	...
	/** Reference to the pirateScheduler */
	private PirateScheduler scheduler;

	/**
	 * Constructs the pirateSchedulerGUI and sets up the GUI components.
	 */
	public PirateSchedulerGUI() {
		super();
		...
		try {
			scheduler = new PirateScheduler(getFileName(true));
	...
	/**
	 * Starts the pirate Scheduler program.
	 * 
	 * @param args command line arguments
	 */
	public static void main(String[] args) {
		new PirateSchedulerGUI();
	}
	...
	private class SchedulerPanel extends JPanel implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == btnAddCourse) {
				...
				if (row == -1) {
					JOptionPane.showMessageDialog(PirateSchedulerGUI.this, "No course selected in the catalog.");
				} else {
					try {
						if (!scheduler.addCourse(tableCatalog.getValueAt(row, 0).toString(),
								tableCatalog.getValueAt(row, 1).toString())) {
							JOptionPane.showMessageDialog(PirateSchedulerGUI.this, "Course doesn't exist.");
						}
					} catch (IllegalArgumentException iae) {
						JOptionPane.showMessageDialog(PirateSchedulerGUI.this, iae.getMessage());
					}
				}
				updateTables();
				...
				if (row == -1) {
					JOptionPane.showMessageDialog(PirateSchedulerGUI.this, "No item selected in the schedule.");
				} else {
					scheduler.removeActivity(row);
				}
				updateTables();
				...
				try {
					...
					JOptionPane.showMessageDialog(PirateSchedulerGUI.this, "Invalid title.");
				}
				...
				try {
					...
					if (eventMeetingDays.length() == 0) {
						JOptionPane.showMessageDialog(PirateSchedulerGUI.this,
					...
					if (hourIdx == -1) {
						JOptionPane.showMessageDialog(PirateSchedulerGUI.this, "The event times are invalid.");
						return;
					}
					...
					if (weeklyRepeatIdx == -1) {
						JOptionPane.showMessageDialog(PirateSchedulerGUI.this, "The weekly repeat is invalid.");
						return;
					}
					eventWeeklyRepeat = comboWeeklyRepeat.getItemAt(weeklyRepeatIdx);

					scheduler.addEvent(txtEventTitle.getText(), eventMeetingDays, eventStartTime, eventEndTime,
							eventWeeklyRepeat, txtDetails.getText());
					...
					JOptionPane.showMessageDialog(PirateSchedulerGUI.this, message);
				}
			}

			PirateSchedulerGUI.this.repaint();
			PirateSchedulerGUI.this.validate();
		}
